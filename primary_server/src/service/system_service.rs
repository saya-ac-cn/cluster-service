...
use crate::domain::mapper::user_mapper::UserMapper;
...
use crate::domain::vo::total_log::TotalLogVO;
...
impl SystemService {
    ...
    pub async fn generate_token(&self, req: &HttpRequest, user: &User,platform:&str) -> Result<SignInVO> {
        ...
        let today_plan_result_warp = pool!().query_decode::<Vec<HashMap<String, String>>>(query_today_plan_sql, vec![to_value!(user.account.clone()),to_value!(user.account.clone())]).await;
        if today_plan_result_warp.is_ok(){
            sign_vo.plan = Some(today_plan_result_warp.unwrap());
        }

        // 查询最近的一次操作日志
        let log_warp = LogMapper::select_recently(pool!(), &user.account.clone().unwrap()).await;
        if log_warp.is_ok() {
    ...
    pub async fn user_page(&self, arg: &UserPageDTO) -> Result<Page<UserVO>> {
        ...
        let count_result = UserMapper::select_count(pool!(), &arg, &extend).await;
        ...
        let page_result = UserMapper::select_page(pool!(), &arg, &extend).await;
    ...
    pub async fn user_add(&self, arg: &UserDTO) -> Result<u64> {
        ...
        let write_result = User::insert(pool!(),&user).await;
    ...
    pub async fn user_edit(&self, req: &HttpRequest, arg: &UserDTO) -> Result<u64> {
        ...
        let result = UserMapper::update_user(pool!(), &user_edit).await;//CONTEXT.primary_rbatis.update_by_column(User::account(),&user_edit).await?;
        if result.is_err() {
    ...
    pub async fn user_detail(&self, arg: &UserDTO) -> Result<UserVO> {
        ...
        let query_user_wrap = User::select_by_account(pool!(),  &account.clone()).await;
        if query_user_wrap.is_err() {
            error!("查询用户异常：{}",query_user_wrap.unwrap_err());
            return Err(Error::from("查询用户失败!"));
        }
    ...
    pub async fn log_get_type(&self) -> Result<Vec<LogTypeVO>> {
        let query_result = LogTypeMapper::select_all(pool!()).await;
        if query_result.is_err() {
    ...
    pub async fn log_page(&self, req: &HttpRequest,param: &LogPageDTO) -> Result<Page<LogVO>>  {
        ...
        let count_result = LogMapper::select_count(pool!(), &arg,&extend).await;
        if count_result.is_err(){
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(result.page_size);
        let page_result = LogMapper::select_page(pool!(), &arg,&extend).await;
        if page_result.is_err() {
    ...
    //     let current_month_log_result_warp = pool!().fetch_decode::<u64>(query_current_month_log_sql, vec![to_value!(user_info.organize),to_value!(month.as_str())]).await;
    ...
    //     let compute_result_warp = pool!().fetch_decode::<Vec<TotalPre6MonthVO>>(query_sql.as_str(), vec![]).await;
    ...
    //     let param:Vec<Bson> = Vec::new();
    //     let primary_count_result_warp = pool!().fetch_decode(query_primary_count_sql.as_str(), vec![]).await;
    //     // 异常健壮处理
    ...
    pub async fn add_plan(&self, req: &HttpRequest,arg: &PlanDTO) -> Result<u64> {
        ...
        let write_result = Plan::insert(pool!(),&plan).await;
        if  write_result.is_err(){
    ...
    pub async fn edit_plan(&self, req: &HttpRequest,arg: &PlanDTO) -> Result<u64> {
        ...
        let result = PlanMapper::update_plan(pool!(),&plan).await;
        if result.is_err() {
    ...
    pub async fn plan_page(&self, req: &HttpRequest, param: &PlanPageDTO) -> Result<Page<PlanVO>>  {
        ...
        let count_result = PlanMapper::select_count(pool!(), &arg,&extend).await;
        if count_result.is_err(){
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(result.page_size);
        let page_result = PlanMapper::select_page(pool!(), &arg,&extend).await;
        if page_result.is_err() {
    ...
    pub async fn advance_finish_plan(&self, req: &HttpRequest,id: &u64) -> Result<u64> {
        ...
        if 1 == plan_exist.cycle.unwrap() {
            ...
            let write_result = PlanArchive::insert(pool!(),&plan_archive).await;
            if  write_result.is_err(){
    ...
    pub async fn plan_archive_page(&self, req: &HttpRequest, param: &PlanArchivePageDTO) -> Result<Page<PlanArchiveVO>>  {
        ...
        let count_result = PlanArchiveMapper::select_count(pool!(), &arg,&extend).await;
        if count_result.is_err(){
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(result.page_size);
        let page_result = PlanArchiveMapper::select_page(pool!(), &arg,&extend).await;
        if page_result.is_err() {
    ...
    pub async fn db_dump_log_page(&self, req: &HttpRequest,param: &DbDumpLogPageDTO) -> Result<Page<DbDumpLogVO>>  {
        ...
        let count_result = DbDumpLogMapper::select_count(pool!(), &param,&extend).await;
        if count_result.is_err(){
        ...
        extend.page_no = Some((result.page_no-1)*result.page_size);
        extend.page_size = Some(result.page_size);
        let page_result = DbDumpLogMapper::select_page(pool!(), &param,&extend).await;
        if page_result.is_err() {
    ...
    //     let compute_result_warp = pool!().fetch_decode::<Vec<PlanArchiveVO>>(query_sql.as_str(), vec![]).await;
    //     if compute_result_warp.is_err(){
