...
impl TokenUtils {
    ...
    pub async fn create_token() -> String{
        ...
        CONTEXT.redis_service.set_string_ex(&format!("{:}:{:}", &util::REQUEST_TOKEN_PREFIX, &token),&Local::now().to_string(),Some(Duration::from_secs(86400))).await;
        token
    }
    ...
    pub async fn check_token(token:Option<String>) -> Option<bool>{
        ...
        return match CONTEXT.redis_service.delete(&format!("{:}:{:}", &util::REQUEST_TOKEN_PREFIX, &token)).await {
            Ok(result) => {
                match 1 == result {
                    true => {
                        Some(true)
