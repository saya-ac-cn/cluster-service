...
pub async fn do_execute_mysqldump(){
    ...
    match code {
        Some(code) => {
            ...
            let write_result = DbDumpLog::insert(pool!(),&log).await;
...
pub async fn do_plan_notice(date:String) {
    ...
    // 查询所有的用户信息，并放入map中
    let user_query_result = User::select_all(pool!()).await;
    ...
    let list_result = PlanMapper::select_list(pool!(), &query_where).await;
...
pub async fn do_undone_plan_notice(){
    println!("------");
    // 查询截止到今日都还没有完成的任务
    // let query_list_result = PlanArchiveMapper::select_undone_list(pool!()).await;
...
impl Scheduler {
    ...
    pub async fn start() {
        // 查询现存所有活跃的提醒计划
        let plan_result= Plan::select_all(pool!()).await;
